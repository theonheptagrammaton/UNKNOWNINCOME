  ...
  return (
    <section className="flex flex-col gap-4 rounded border border-line bg-graphite p-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex overflow-hidden rounded border border-line">
            {(["off", "paper", "live"] as BotMode[]).map((m) => {
              const active = mode === m;
              const disabled = m === "live" && !(status?.live_enabled ?? false);
              return (
                <button
                  key={m}
                  type="button"
                  ...
                  title={
                    disabled ? "Live is disabled until Phase 7 (no live adapter yet)" : undefined
                  }
                  onClick={() => onMode(m)}
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-fog-faint">
        ...
        <span>LIVE disabled until Phase 7</span>
      </div>
    </section>
  );
}
...
export function TradeDeck() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [st, pf, sg, dc, str] = await Promise.all([
        fetchBotStatus(),
        fetchPortfolio(),
        fetchSignals(),
        fetchDecisions(),
